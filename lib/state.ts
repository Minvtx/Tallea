...
import { getWorldStore } from "@/lib/world-store";
 ...
 */
export async function writeCycleOutput(cycle: CycleOutput): Promise<string> {
  return getWorldStore().writeCycleOutput(cycle);
}

/**
 * Apply a cycle to the given state, persist current_state.json, upsert a
 * compact TimelineEvent to timeline.json, and upsert the derived
 ...
 */
export async function applyAndPersistCycle(
  ...
  await store.writeCurrentWorldState(nextState);
  ...
  await store.appendOrReplaceTimelineEvent(timelineEvent);
  ...
  await store.appendOrReplaceLogEntries(cycle.cycle_id, logEntries);

  return { nextState, timelineEvent, logEntries };
}

/**
 * Upsert a per-cycle GenerationMetadata record to data/generation_log.json.
 * Idempotent under reruns of the same cycle_id (replaces in place).
 */
export async function appendGenerationMetadata(
  meta: GenerationMetadata,
): Promise<void> {
  await getWorldStore().appendOrReplaceGenerationMetadata(meta);
}
...
export async function resetWorldState(): Promise<void> {
  await getWorldStore().resetMutableWorldState();
}
